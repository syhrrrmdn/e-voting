...
function LoginForm() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      setErrorMsg('Gagal menginisialisasi login Google.');
      setLoading(null);
    }
  };
    ...
    <div className="max-w-md w-full space-y-8 z-10">
      ...
      <div className="text-center animate-fade-in">
        ...
        <h2 className="text-3xl font-extrabold text-white tracking-tight">
          Muda<span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-cyan-400">Vote</span>
        </h2>
        <p className="mt-2 text-sm text-slate-400">
          Platform E-Voting Organisasi Modern & Aman
        </p>
      </div>
      ...
      <div className="bg-slate-800/80 border border-slate-700/50 backdrop-blur-xl rounded-3xl p-8 shadow-2xl animate-scale-in">
        ...
        <form onSubmit={handleCredentialsLogin} className="space-y-5">
          ...
          <div>
            <label htmlFor="password" className="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-2">
              Kata Sandi (Hanya jika terdaftar dengan password)
            </label>
            <div className="relative">
              <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-400">
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
        ...
        <div className="my-6 flex items-center justify-between">
          <span className="w-1/5 border-b border-slate-700/60" />
        ...
        <button
          ...
          disabled={loading !== null}
          className="w-full py-3 px-4 rounded-xl border border-slate-700 hover:bg-slate-700/40 text-slate-200 font-medium text-sm transition-all duration-150 flex items-center justify-center gap-3 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed mb-8"
        >
          {loading === 'google' ? (
            <svg className="animate-spin h-5 w-5 text-white" fill="none" viewBox="0 0 24 24">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
            ...
            <svg className="w-5 h-5" viewBox="0 0 24 24" width="24" height="24">
              <path fill="#EA4335" d="M12.24 10.285V14.4h6.887c-.648 2.41-2.519 4.114-5.187 4.114-3.513 0-6.39-2.877-6.39-6.39s2.877-6.39 6.39-6.39c1.637 0 3.126.619 4.26 1.748L21.32 4.37C19.06 2.25 15.93 1 12.24 1 5.92 1 1 5.92 1 12.24s4.92 11.24 11.24 11.24c5.78 0 10.87-4.14 10.87-11.24 0-.74-.08-1.32-.23-1.955H12.24z" />
            </svg>
          )}
      ...
      <p className="text-center text-xs text-slate-500">
        MudaVote &copy; {new Date().getFullYear()}. All rights reserved.
      </p>
    </div>
  );
}
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-900 px-4 py-12 relative overflow-hidden font-sans">
      ...
      <Suspense fallback={
        <div className="max-w-md w-full space-y-8 z-10 flex flex-col items-center justify-center">
          ...
          <p className="text-slate-400 text-sm font-medium">Memuat Halaman Masuk...</p>
        </div>
      }>
        <LoginForm />
      </Suspense>
